...
# =====================================================================
# CONSIGNES DU KATA : ALGORITHME DE FORD-FULKERSON
# =====================================================================
# L'algorithme de Ford-Fulkerson résout le problème du flot maximum 
...
# un parcours en profondeur (DFS - Depth-First Search) pour trouver 
# des chemins augmentants.
#
# Règles de l'algorithme :
# 1. Commencer avec un flot de zéro sur toutes les arêtes.
# 2. Utiliser DFS pour trouver un chemin augmentant (une capacité 
#    disponible) de la source au puits.
# 3. Faire un calcul de "goulot d'étranglement" pour savoir quel flot 
#    maximum peut être envoyé sur ce chemin (la capacité résiduelle minimale).
# 4. Augmenter le flot le long du chemin (ce qui réduit la capacité résiduelle).
# 5. Ajouter cette même valeur à la capacité des arêtes inverses pour 
#    permettre "d'annuler" ou de repousser un flot si nécessaire.
...
class Graph:
    ...
    def dfs(self, source: int, sink: int, parent: list[int]) -> bool:
        """
        Trouve un chemin augmentant de la source au puits en utilisant DFS.
    ...
    def ford_fulkerson(self, source: int, sink: int) -> int:
        ...
        # Tant qu'il existe un chemin augmentant (trouvé via DFS)
...
class TestFordFulkerson(unittest.TestCase):
    ...
    def test_example_from_kata(self):
        ...
        self.assertEqual(g.ford_fulkerson(0, 5), 8)
    ...
    def test_simple_linear_graph(self):
        ...
        self.assertEqual(g.ford_fulkerson(0, 2), 15)

    def test_graph_with_bottleneck(self):
        # Graphe avec un goulot d'étranglement restrictif
        g = Graph(4)
        g.add_edge(0, 1, 100)
        g.add_edge(1, 2, 5)    # Goulot d'étranglement
        g.add_edge(2, 3, 100)
        
        self.assertEqual(g.ford_fulkerson(0, 3), 5)

    def test_disconnected_graph(self):
        # Graphe où la source et le puits n'ont aucun chemin possible
        g = Graph(4)
        g.add_edge(0, 1, 10)
        g.add_edge(2, 3, 10)
        
        self.assertEqual(g.ford_fulkerson(0, 3), 0)
